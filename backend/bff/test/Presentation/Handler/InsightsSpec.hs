module Presentation.Handler.InsightsSpec (spec) where
...
viewerWithoutInsights :: AuthenticatedUser
viewerWithoutInsights =
  AuthenticatedUser
...
insightsRequest :: Maybe Text -> SRequest
insightsRequest maybeToken =
  SRequest
    { simpleRequest =
        setPath
          defaultRequest
...
insightByIdentifierRequest :: Maybe Text -> SRequest
insightByIdentifierRequest maybeToken =
  SRequest
    { simpleRequest =
        setPath
          defaultRequest
...
spec :: Spec
spec = describe "Presentation.Handler.Insights" $ do
  describe "GET /insights" $ do
    it "returns 401 when Authorization header is missing" $ do
      response <- runSession (srequest (insightsRequest Nothing)) testApp
    ...
    it "returns 401 when Authorization header has invalid token" $ do
      ...
      response <- runSession (srequest (insightsRequest (Just invalidToken))) testApp
      simpleStatus response `shouldBe` status401

    it "returns 403 when token lacks insights:read permission" $ do
      tokenText <- issueTestToken viewerWithoutInsights
      response <- runSession (srequest (insightsRequest (Just tokenText))) testApp
  ...
  describe "GET /insights/some-identifier" $ do
    it "returns 401 when Authorization header is missing" $ do
      response <- runSession (srequest (insightByIdentifierRequest Nothing)) testApp
      simpleStatus response `shouldBe` status401
