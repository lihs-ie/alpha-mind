module Presentation.Handler.AuditSpec (spec) where
...
viewerWithoutAudit :: AuthenticatedUser
viewerWithoutAudit =
  AuthenticatedUser
...
auditRequest :: Maybe Text -> SRequest
auditRequest maybeToken =
  SRequest
    { simpleRequest =
        setPath
          defaultRequest
...
spec :: Spec
spec = describe "Presentation.Handler.Audit" $ do
  describe "GET /audit" $ do
    it "returns 401 when Authorization header is missing" $ do
      response <- runSession (srequest (auditRequest Nothing)) testApp
    ...
    it "returns 401 when Authorization header has invalid token" $ do
      ...
      response <- runSession (srequest (auditRequest (Just invalidToken))) testApp
      simpleStatus response `shouldBe` status401

    it "returns 403 when token lacks audit:read permission" $ do
      tokenText <- issueTestToken viewerWithoutAudit
      response <- runSession (srequest (auditRequest (Just tokenText))) testApp
