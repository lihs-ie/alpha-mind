...
def _from_signal_dispatch(signal_dispatch: SignalDispatch) -> dict[str, Any]:
    ...
    return {
        ...
        "publishedEvent": (
            signal_dispatch.published_event.value if signal_dispatch.published_event is not None else None
        ),
        "reasonCode": (signal_dispatch.reason_code.value if signal_dispatch.reason_code is not None else None),
