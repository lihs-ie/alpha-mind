...
- Firestore（skill_registry, instruction_profiles, failure_knowledge）
...
- 完了目標: 1サイクル `10分以内`。
- 品質: 仮説に `hypothesisId`, `sourceEvidence`, `skillVersion`, `instructionProfileVersion` を必須付与。
- 安全性: 出力は必ず人手承認を通過させる（自動昇格禁止）。
