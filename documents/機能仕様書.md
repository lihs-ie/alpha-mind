...
8. 監査・可観測性
- 全イベントに `traceId` を付与し、判断根拠とモデル版を保存する。
...
flowchart LR
  ...
  BUS --> DC[svc-data-collector]
  DC -->|market.collected| BUS
  BUS --> IC[svc-insight-collector]
  IC -->|insight.collected| BUS
  BUS --> FE[svc-feature-engineering]
  FE -->|features.generated| BUS
  BUS --> AO[svc-agent-orchestrator]
  AO -->|hypothesis.proposed| BUS
  BUS --> SG[svc-signal-generator]
  SG -->|signal.generated| BUS
  BUS --> HL[svc-hypothesis-lab]
  HL -->|hypothesis.backtested| BUS
  BUS --> PL[svc-portfolio-planner]
  PL -->|orders.proposed| BUS
  BUS --> RK[svc-risk-guard]
  RK -->|orders.approved| BUS
  RK -->|orders.rejected| BUS
  BUS --> EX[svc-execution]
  EX -->|orders.executed| BUS
  BUS --> AU[svc-audit-log]
  ...
  UI[Web Console] --> BFF[svc-bff API Gateway]
  BFF --> FDB
  BFF --> BUS
```
...
- AI探索運用
- AIは戦略案の探索と改善ループを担当し、最終判断は運用者が行う。
...
| 観測/監査 | Cloud Logging + Error Reporting + traceId | ローカルログのみ | 障害復旧と意思決定の追跡を実現。 | ローカルログより検索性・運用性が高い。 | ログ量増でコスト増。保持期間を制御。 | https://cloud.google.com/logging/docs , https://cloud.google.com/error-reporting/docs |
...
|---|---|---|
| 手数料・売買コストで超過収益が消える | 低回転運用、執行前リスクチェック、コスト込み評価 | `svc-portfolio-planner`, `svc-risk-guard`, 検証基盤 |
| 多重検定・過学習 | Walk-forward、DSR/PBO、モデル昇格ゲート | MLflow、検証ジョブ、監査ログ |
| 公表後劣化・シグナル減衰 | 定期再学習、ドリフト監視、モデル差し戻し | `svc-signal-generator`, 監視/通知 |
| レジーム変化 | 特徴量再計算、運用パラメータ切替、kill switch | `svc-feature-engineering`, 設定管理、`svc-risk-guard` |
...
4. 監査ログ画面
- 表示項目: イベント時系列、traceId、モデルバージョン、判断理由
